...
def scrape_from_WhitePagesCanada(street_name, houses_on_street, whole_street):
    ...
    for url in person_url:
        ...
        house_number, street_name = split_address(address.text)
...
def scrape_from_411(street_name, houses_on_street, whole_street):
    ...
    for FullPerson in content.findAll('div', attrs={"class": "c411Listing jsResultsList"}):
        ...
        house_number, street_name = split_address(short_address)
        if (house_number in houses_on_street) or whole_street:
            people.append([house_number, street_name, phone_number, name])
    scrape_results = pandas.DataFrame.from_records(people, columns=["House Number", "Address", "Phone Number", "Name"])
    return scrape_results
...
def getInfoFromConsole():
    ...
    scrape_from_411_results = scrape_from_411(streetName, [], True)
    scrape_from_whitepagescanada_results = scrape_from_WhitePagesCanada(streetName, [], True)
...
def get_info_from_file():
    ...
    for street in unique_streets:
        ...
        df_canada_white_pages = scrape_from_WhitePagesCanada(street, houses_on_street, False)
        df_411 = scrape_from_411(street, houses_on_street, False)
        scraped_tables = pandas.concat([df_canada_white_pages, df_411])
        scraped_tables = scraped_tables.drop_duplicates(subset=["House Number", "Phone Number"])
        print("Removed the duplicates")
        scraped_tables_sorted = scraped_tables.sort_values("House Number")
